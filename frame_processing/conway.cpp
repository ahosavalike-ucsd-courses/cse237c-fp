...
void ConwayGame::update(ap_uint<2> move) {
    ...
    for (ap_uint<CONWAY_BIT_WIDTH + 1> i = 0; i < CONWAY_FRAME_WIDTH; i++) {
        for (ap_uint<CONWAY_BIT_WIDTH> j = 0; j < CONWAY_FRAME_HEIGHT; j++) {
            ...
            if (chaos_set && (y >= CONWAY_FRAME_HEIGHT ? ap_uint<CONWAY_BIT_WIDTH>(ap_uint<CONWAY_BIT_WIDTH>(CONWAY_FRAME_HEIGHT) - y) : y) == j && chaos.range(CONWAY_BIT_WIDTH - 1, 0) == i) {
                world[i][j].live ^= 1;
                world[i][j].chaos = 1;
            }
        }
...
void ConwayGame::draw_world(hls::stream<streaming_data>& input, hls::stream<streaming_data>& output) {
    ...
    static hls::stream<streaming_data> temps[CONWAY_BIT_WIDTH * CONWAY_FRAME_HEIGHT - 1];
    int counter = 0;
    g.draw(RECTANGLE_FILLED, colors[world[0][0].live][world[0][0].chaos], Point(0, 0), Point(0, 0) + CONWAY_FACTOR, input, temps[0]);
    for (ap_uint<CONWAY_BIT_WIDTH + 1> i = 0; i < CONWAY_FRAME_WIDTH; i++) {
        for (ap_uint<CONWAY_BIT_WIDTH> j = 0; j < CONWAY_FRAME_HEIGHT; j++) {
            if (i == 0 && j == 0) continue;
            if (i == (CONWAY_BIT_WIDTH - 1) && j == (CONWAY_FRAME_HEIGHT - 1)) break;
    ...
    ap_uint<CONWAY_BIT_WIDTH> x = FRAME_WIDTH - 1, y = FRAME_HEIGHT - 1;
